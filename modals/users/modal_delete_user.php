...
if (isset($_POST['user_id'])) {
  ...
  if ($result) {
    while ($row = mysqli_fetch_assoc($result)) {
      ?>
      <div class="modal fade" id="deleteDataUserModal" tabindex="-1" role="dialog" aria-labelledby="requestModalLabel"
        aria-hidden="true">
        <div class="modal-dialog modal-l" role="document">
          <div class="modal-content">
            <div class="modal-header">
              <h5 class="modal-title">Delete User ID: <?php echo $row['user_id']; ?></h5>
...
<script>
  ...
  $(document).ready(function () {
    $('#deleteDataUserModal form').submit(function (event) {
      ...
      $.ajax({
        type: 'POST',
        url: '/appointment/controllers/admin/delete_user_process.php',
        data: formData,
        success: function (response) {
          ...
          if (response.success) {
            ...
            $('#deleteDataUserModal').modal('hide');
